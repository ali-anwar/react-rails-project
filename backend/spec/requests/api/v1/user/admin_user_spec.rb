  ...
  describe 'GET /users' do
    context 'listing' do
      ...
      it 'returns listing' do
        ...
        expect(json.collect { |user| user['id'] }.include?(admin_user.id)).to be true
        expect(json.collect { |user| user['name'] }.include?(admin_user.name)).to be true
        expect(json.collect { |user| user['email'] }.include?(admin_user.email)).to be true
        expect(json.collect { |user| user['role'] }.include?(admin_user.role)).to be true
      end
    end
  end
